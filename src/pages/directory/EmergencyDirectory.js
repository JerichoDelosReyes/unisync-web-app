...
const EmergencyDirectory = () => {
    ...
    const fetchContacts = async () => {
      try {
        ...
        const contactsWithIcons = contacts.map(contact => ({
          ...contact,
          icon: iconMap[contact.icon] || Shield
        }));
        setEmergencyContacts(contactsWithIcons);
      } catch (error) {
        console.error('Error fetching emergency contacts:', error);
        // Fallback to default contacts if Firebase fetch fails
  ...
  return (
    <div className="directory-page">
      ...
      {/* Contact Cards */}
      {loading ? (
        <Loader text="Loading emergency contacts..." />
      ) : (
