  ...
  useEffect(() => {
    const fetchBuildings = async () => {
      try {
        ...
        const buildingsWithIcons = buildingsData.map(building => ({
          ...building,
          icon: iconMap[building.icon] || Building2
        }));
        setBuildings(buildingsWithIcons);
        if (buildingsWithIcons.length > 0) {
          setExpandedBuilding(buildingsWithIcons[0].id);
        }
      } catch (error) {
        console.error('Error fetching buildings:', error);
        // Fallback to default buildings if Firebase fetch fails
        setBuildings([
          {
            id: 'new',
            name: 'New Building',
